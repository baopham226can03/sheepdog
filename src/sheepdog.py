...
parser.add_argument('--attack_mode', default='style', choices=['style', 'sentiment'], type=str,
                    help='Which reframing/attack mode to use for training (style or sentiment)')
parser.add_argument('--adv_test_file', default=None, type=str, help='Path to adversarial test .pkl file (overrides default)')
parser.add_argument('--sentiment_dir', default='data/sentiment_data', type=str, help='Directory for sentiment-generated files')
...
def train_model(tokenizer, max_len, n_epochs, batch_size, datasetname, iter):
    ...
    x_train, x_test, x_test_res, y_train, y_test = load_articles(datasetname, adv_test_file=args.adv_test_file)
    ...
    for epoch in range(n_epochs):
        model.train()
        x_train_res1, x_train_res2, y_train_fg, y_train_fg_m, y_train_fg_t = load_reframing(args.dataset_name, mode=args.attack_mode, sentiment_dir=args.sentiment_dir, target_len=len(x_train))
